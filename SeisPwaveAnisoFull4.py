...
def run_modeling(params, enable_fluid_sub, seismic_cmap, selected_angle, max_angle, angle_step, freq, azimuth_step):
    """Run the modeling with given parameters"""
...
def display_results(results, seismic_cmap, selected_angle, max_angle):
    """Display modeling results in main workspace"""
    ...
    with tab1:
        ...
        with col1:
            fig_orig = go.Figure(data=[go.Surface(
                ...
                colorscale='Jet',
                cmin=min(np.min(results['reflectivity_orig']), np.min(results['reflectivity_sub'])),
                cmax=max(np.max(results['reflectivity_orig']), np.max(results['reflectivity_sub']))
            )])
            fig_orig.update_layout(title="Original Response", height=500)
        ...
        with col2:
            ...
            fig_sub.update_layout(title="Fluid-Substituted Response", height=500)
    ...
    with tab2:
        ...
        ax1.set(xlabel='Azimuth (degrees)', ylabel='Reflectivity', title=f'AVAZ Reflectivity at {selected_angle}° Incidence')
        ax1.grid(True)
        ax1.legend()
    ...
    with tab4:
        ...
        fig6.update_layout(title="3D Reflectivity Difference", height=600)
        st.plotly_chart(fig6, use_container_width=True)
    
    with tab5:
        st.header("Velocity Parameters Comparison")
        
        # Create a DataFrame for better display
        params_df = pd.DataFrame({
            'Parameter': ['Vp (m/s)', 'Vs (m/s)', 'Density (g/cc)', 'Epsilon', 'Delta', 'Gamma'],
            'Upper Layer (Original)': [
                results['vp_orig'][0], results['vs_orig'][0], results['d_orig'][0],
                results['e_orig'][0], results['dlt_orig'][0], results['g_orig'][0]
            ],
            ...
            'Target Layer (Substituted)': [
                results['vp_sub'][1], results['vs_sub'][1], results['d_sub'][1],
                results['e_sub'][1], results['dlt_sub'][1], results['g_sub'][1]
            ],
            'Lower Layer (Original)': [
                results['vp_orig'][2], results['vs_orig'][2], results['d_orig'][2],
                results['e_orig'][2], results['dlt_orig'][2], results['g_orig'][2]
            ]
        })
        
        st.dataframe(params_df.style.format("{:.2f}"))
        
        # Show percentage changes for target layer
        st.subheader("Target Layer Changes After Fluid Substitution")
        changes_df = pd.DataFrame({
            'Parameter': ['Vp', 'Vs', 'Density', 'Epsilon', 'Delta', 'Gamma'],
            'Original Value': [
                results['vp_orig'][1], results['vs_orig'][1], results['d_orig'][1],
                results['e_orig'][1], results['dlt_orig'][1], results['g_orig'][1]
            ],
...
def main():
    ...
    if modeling_mode == "Manual Input":
        ...
        with st.sidebar:
            ...
            max_angle = st.slider("Maximum Angle (deg)", 1, 90, 60)
            angle_step = st.slider("Angle Step (deg)", 1, 10, 2)
            ...
            selected_angle = st.selectbox("Select angle for 2D comparison", 
                                        options=np.arange(0, max_angle+1, angle_step),
                                        index=3)
        ...
        if st.sidebar.button("Run Modeling"):
            results = run_modeling(params, enable_fluid_sub, seismic_cmap, selected_angle, 
                                 max_angle, angle_step, freq, azimuth_step)
            display_results(results, seismic_cmap, selected_angle, max_angle)
        ...
        with st.sidebar:
            ...
            if uploaded_file is not None:
                try:
                    ...
                    # Show depth range visualization in main workspace
                    ...
                    st.subheader("Acquisition Parameters")
                    max_angle = st.slider("Maximum Angle (deg)", 1, 90, 60, key="excel_max_angle")
                    angle_step = st.slider("Angle Step (deg)", 1, 10, 2, key="excel_angle_step")
            ...
            if uploaded_file is not None and st.sidebar.button("Run Modeling with Excel Data"):
                try:
                    # Check if we have valid depth ranges
                    if len(st.session_state.depth_ranges) != 3:
                        st.error("Please set valid depth ranges for all three layers")
                        return
                    
                    # Process Excel data with individual layer ranges
                    params = process_excel_data(uploaded_file, st.session_state.depth_ranges)
                    ...
                    if params is not None:
                        ...
                        if enable_fluid_sub:
                            params.update({
                                ...
                                'Gm': Gm,
                                'Kf': Kf,
                                'new_fluid_density': new_fluid_density
                            })
        ...
        if uploaded_file is not None:
            if hasattr(st.session_state, 'depth_ranges'):
                st.header("Depth Range Visualization")
                plot_depth_ranges(st.session_state.depth_ranges, 
