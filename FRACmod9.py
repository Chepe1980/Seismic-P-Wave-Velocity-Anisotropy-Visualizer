...
def process_csv_data(uploaded_file, depth_ranges):
    ...
    try:
        ...
        # Ensure required columns exist (check for variations)
        required_cols = ['DEPTH', 'VP', 'VS', 'DENSITY', 'EPSILON', 'DELTA', 'GAMMA']
        missing_cols = []
        
        # Check each required column with possible variations
        col_mapping = {}
        for req_col in required_cols:
            found = False
            for df_col in df.columns:
                if req_col in df_col or req_col.lower() in df_col.lower():
        ...
        if missing_cols:
            ...
            st.info("Please ensure your CSV has columns named: DEPTH, VP, VS, DENSITY, EPSILON, DELTA, GAMMA")
            return None
        
        # Convert numeric columns
        for col in ['DEPTH', 'VP', 'VS', 'DENSITY', 'EPSILON', 'DELTA', 'GAMMA']:
            df[col] = pd.to_numeric(df[col], errors='coerce')
        
        # Drop rows with NaN in required columns
        df = df.dropna(subset=['DEPTH', 'VP', 'VS', 'DENSITY'])
        ...
        for i, (min_depth, max_depth) in enumerate(depth_ranges, 1):
            ...
            params[f'e{i}'] = float(layer_df['EPSILON'].median()) if 'EPSILON' in layer_df.columns else 0.0
            params[f'g{i}'] = float(layer_df['GAMMA'].median()) if 'GAMMA' in layer_df.columns else 0.0
            params[f'dlt{i}'] = float(layer_df['DELTA'].median()) if 'DELTA' in layer_df.columns else 0.0
    ...
    if modeling_mode == "Manual Input":
        ...
        with st.sidebar:
            ...
            for i, layer in enumerate(layers, 1):
                ...
                params[f'vp{i}'] = st.number_input(f"Vp{i} (m/s)", value=5500 if i!=2 else 4742, key=f"vp_{i}")
                params[f'vs{i}'] = st.number_input(f"Vs{i} (m/s)", value=3600 if i!=2 else 3292, key=f"vs_{i}")
                params[f'd{i}'] = st.number_input(f"Density{i} (g/cc)", value=2.6 if i!=2 else 2.4, step=0.1, key=f"d_{i}")
                params[f'e{i}'] = st.number_input(f"ε{i}", value=0.1 if i==1 else (-0.01 if i==2 else 0.2), step=0.01, key=f"e_{i}")
                params[f'g{i}'] = st.number_input(f"γ{i}", value=0.05 if i==1 else (-0.05 if i==2 else 0.15), step=0.01, key=f"g_{i}")
                params[f'dlt{i}'] = st.number_input(f"δ{i}", value=0.0 if i==1 else (-0.13 if i==2 else 0.1), step=0.01, key=f"dlt_{i}")
            ...
            selected_angle = st.slider(
                ...
                key="avaz_selected_angle"
            )
            freq = st.slider("Wavelet Frequency (Hz)", 10, 100, 45, key="avaz_freq")
            azimuth_step = st.slider("Azimuth Step (deg)", 1, 30, 10, key="avaz_azimuth_step")
            
            st.subheader("Fluid Substitution Parameters")
            enable_fluid_sub = st.checkbox("Enable Fluid Substitution", True, key="avaz_enable_fluid")
            if enable_fluid_sub:
                params['phi'] = st.slider("Porosity (ϕ)", 0.01, 0.5, 0.2, 0.01, key="avaz_phi")
                params['Km'] = st.number_input("Mineral Bulk Modulus (GPa)", 10.0, 100.0, 37.0, 1.0, key="avaz_km")
                params['Gm'] = st.number_input("Mineral Shear Modulus (GPa)", 10.0, 100.0, 44.0, 1.0, key="avaz_gm")
                params['Kf'] = st.number_input("Fluid Bulk Modulus (GPa)", 0.1, 5.0, 2.2, 0.1, key="avaz_kf")
                params['new_fluid_density'] = st.number_input("New Fluid Density (g/cc)", 0.1, 1.5, 1.0, 0.1, key="avaz_new_fluid_density")
            
            # Add button to show P-wave anisotropy section
            show_anisotropy = st.checkbox("Show P-Wave Anisotropy Section", False, key="avaz_show_anisotropy")
            
            if st.button("Run Modeling", key="avaz_run_button"):
        ...
        with st.sidebar:
            ...
            uploaded_file = st.file_uploader("Upload CSV file", type=["csv"], key="avaz_csv_uploader")
            
            if uploaded_file is not None:
                try:
                    # Reset file pointer and read with preview
                    uploaded_file.seek(0)
                    ...
                    # Read full CSV to get depth range
                    df_full = pd.read_csv(uploaded_file)
                    df_full.columns = df_full.columns.str.upper().str.strip()
                    
                    if 'DEPTH' not in df_full.columns:
                        st.error("CSV must contain 'DEPTH' column")
                        st.stop()
                    
                    # Convert depth to numeric
                    df_full['DEPTH'] = pd.to_numeric(df_full['DEPTH'], errors='coerce')
                    df_full = df_full.dropna(subset=['DEPTH'])
                    
                    if len(df_full) == 0:
                        st.error("No valid depth data found")
                        st.stop()
                    
                    min_depth = float(df_full['DEPTH'].min())
                    max_depth = float(df_full['DEPTH'].max())
                    ...
                    for i, layer in enumerate(layers, 1):
                        ...
                        with col1:
                            min_val = st.number_input(
                                ...
                                min_value=min_depth, 
                                max_value=max_depth,
                                value=float(default_ranges[i-1][0]),
                                key=f"avaz_min_depth_{i}"
                            )
                        with col2:
                            max_val = st.number_input(
                                ...
                                min_value=min_depth, 
                                max_value=max_depth,
                                value=float(default_ranges[i-1][1]),
                                key=f"avaz_max_depth_{i}"
                            )
                        # Ensure valid range
                        if min_val >= max_val:
                    ...
                    freq = st.slider("Wavelet Frequency (Hz)", 10, 100, 45, key="avaz_csv_freq")
                    azimuth_step = st.slider("Azimuth Step (deg)", 1, 30, 10, key="avaz_csv_azimuth_step")
                    
                    st.subheader("Fluid Substitution Parameters")
                    enable_fluid_sub = st.checkbox("Enable Fluid Substitution", True, key="avaz_csv_fluid_sub")
                    if enable_fluid_sub:
                        phi = st.slider("Porosity (ϕ)", 0.01, 0.5, 0.2, 0.01, key="avaz_csv_phi")
                        Km = st.number_input("Mineral Bulk Modulus (GPa)", 10.0, 100.0, 37.0, 1.0, key="avaz_csv_km")
                        Gm = st.number_input("Mineral Shear Modulus (GPa)", 10.0, 100.0, 44.0, 1.0, key="avaz_csv_gm")
                        Kf = st.number_input("Fluid Bulk Modulus (GPa)", 0.1, 5.0, 2.2, 0.1, key="avaz_csv_kf")
                        new_fluid_density = st.number_input("New Fluid Density (g/cc)", 0.1, 1.5, 1.0, 0.1, key="avaz_csv_new_fluid_density")
                    
                    show_anisotropy = st.checkbox("Show P-Wave Anisotropy Section", False, key="avaz_csv_show_anisotropy")
                    
                    if st.button("Run Modeling with CSV Data", type="primary", key="avaz_csv_run_button"):
...
def run_fracture_modeling_app():
    ...
    with st.sidebar:
        ...
        data_source = st.radio(
            ...
            key="fracture_data_source"
        )
        
        # Fracture parameters
        st.markdown("### Fracture Parameters")
        crack_density = st.slider("Crack Density", 0.01, 0.2, 0.08, 0.01, key="fracture_crack_density")
        aspect_ratio = st.slider("Aspect Ratio", 0.001, 0.1, 0.01, 0.001, format="%.3f", key="fracture_aspect_ratio")
        
        st.markdown("### Fracture Orientations")
        fracture_azimuth1 = st.slider("Set 1 Azimuth (°)", 0, 180, 0, 5, key="fracture_azimuth1")
        fracture_azimuth2 = st.slider("Set 2 Azimuth (Orthorhombic) (°)", 0, 180, 90, 5, key="fracture_azimuth2")
        fracture_azimuth_mono = st.slider("Set 2 Azimuth (Monoclinic) (°)", 0, 180, 45, 5, key="fracture_azimuth_mono")
...
...
    ...
    if data_source == "Upload CSV":
        uploaded_file = st.file_uploader("Choose a CSV file", type="csv", key="fracture_csv_uploader")
        if uploaded_file is not None:
    ...
    if df is not None:
        ...
        with col1:
            ...
            start_depth = st.number_input("Start Depth (m)", depth_min, depth_max, depth_min, key="fracture_start_depth")
        with col2:
            end_depth = st.number_input("End Depth (m)", depth_min, depth_max, depth_max, key="fracture_end_depth")
        ...
        if start_depth < end_depth:
            ...
            # Run analysis button
            if st.button("🚀 Run Analysis", type="primary", key="fracture_run_button"):
...
class ShearWaveSplittingAnalyzer:
    ...
    def calculate_delta_vs(self, dt, ray_length, vs_background):
        ...
        """
        if ray_length == 0 or vs_background == 0 or dt <= 0:
            return 0
    ...
    with st.sidebar:
        ...
        if data_source == "Upload SWS Data":
            uploaded_file = st.file_uploader(
                ...
                key="sws_file_uploader"
            )
            if uploaded_file is not None:
                if st.button("🚀 Load and Analyze", type="primary", use_container_width=True, key="sws_load_button"):
                    try:
                        uploaded_file.seek(0)
                        df = pd.read_csv(uploaded_file)
            ...
            if st.button("🚀 Generate Synthetic Data", type="primary", use_container_width=True, key="sws_generate_synth"):
            ...
            if 'avaz_results' in st.session_state and st.session_state.avaz_results is not None:
                ...
                fracture_strike_estimate = st.slider("Fracture Strike Estimate (°)", 0, 180, 90, 5, key="sws_fracture_strike")
                q_scale = st.slider("Quality Factor Scale", 0.1, 2.0, 1.2, 0.1, key="sws_q_scale")
                
                if st.button("🚀 Generate SWS from AVAZ Data", type="primary", use_container_width=True, key="sws_generate_avaz"):
