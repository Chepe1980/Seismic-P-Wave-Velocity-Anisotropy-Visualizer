        ...
        with st.sidebar:
            ...
            if uploaded_file is not None:
                ...
                depth_range = st.slider(
                    "Select depth range for analysis",
                    min_value=min_depth,
                    max_value=max_depth,
                ...
                st.subheader("Acquisition Parameters")
                max_angle = st.slider("Maximum Angle (deg)", 1, 90, 60, key="csv_max_angle")
                angle_step = st.slider("Angle Step (deg)", 1, 10, 2, key="csv_angle_step")
                ...
                selected_angle = st.selectbox("Select angle for 2D comparison", 
                                            options=np.arange(0, max_angle+1, angle_step),
                                            index=3, key="csv_selected_angle")
                ...
                seismic_cmap = st.selectbox(
                    "Seismic Colormap",
                    options=['seismic', 'RdBu', 'bwr', 'coolwarm', 'viridis', 'plasma'],
                    index=0, key="csv_seismic_cmap"
                )
                
                # Add button to show P-wave anisotropy section
                show_anisotropy = st.checkbox("Show P-Wave Anisotropy Section", False, key="csv_show_anisotropy")
            
            if uploaded_file is not None and st.sidebar.button("Run Modeling with CSV Data"):
                # Process CSV data
                params = process_csv_data(uploaded_file, depth_range)
