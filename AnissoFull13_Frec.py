...
def plot_horizontal_angles_seismic(results, seismic_cmap, diff_cmap):
...
def plot_horizontal_angles_cwt(results, cwt_scale, cwt_cmap, diff_cmap):
    """Plot CWT analysis with incidence angles arranged horizontally"""
    ...
    # Set global vmax for consistent color scaling (using absolute values for CWT)
    vmax_global = 0
    for angle_idx in range(n_angles):
    ...
    for angle_idx in range(n_angles):
        ...
        im = ax.imshow(cwt_data, aspect='auto',
                      cmap=cwt_cmap,
...
def pwave_anisotropy_section(epsilon, delta, vp0, plot_cmap):
...
def display_results(results, seismic_cmap, diff_cmap, cwt_cmap, selected_angle):
    ...
    # 4. Horizontal Arrangement of Seismic Gathers
    plot_horizontal_angles_seismic(results, seismic_cmap, diff_cmap)
    
    # 5. Horizontal Arrangement of CWT Analysis
    ...
    cwt_results = plot_horizontal_angles_cwt(results, cwt_scale, cwt_cmap, diff_cmap)
    
    # 6. Additional CWT Visualizations
    ...
    im = ax4.imshow(reflectivity_diff.T, aspect='auto', 
                  extent=[0, results['incidence_angles'][-1], 0, 360],
                  cmap=diff_cmap, vmin=-max_diff, vmax=max_diff,
                  origin='lower')
...
def main():
    ...
    diff_cmap = st.sidebar.selectbox(
        "Difference Colormap",
        options=['RdBu', 'seismic', 'bwr', 'coolwarm', 'PuOr', 'PiYG', 'PRGn', 'BrBG', 'RdGy'],
        index=0,
