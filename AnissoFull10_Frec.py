...
def plot_concatenated_seismic(results, seismic_cmap):
    """Plot concatenated seismic gathers for all incidence angles"""
    st.header("Concatenated Seismic Gathers")
    st.markdown("All incidence angles concatenated into single images")
    
    # Original Seismic
    st.subheader("Original Seismic - All Angles Concatenated")
    ...
    concatenated_orig = np.zeros((time_samples * n_angles, n_azimuths))
    
    for i in range(n_angles):
        start_row = i * time_samples
        end_row = (i + 1) * time_samples
        concatenated_orig[start_row:end_row, :] = results['seismic_orig'][i]
    
    # Plot concatenated original seismic
    fig1, ax1 = plt.subplots(figsize=(15, 8))
    vmax = np.abs(concatenated_orig).max()
    im1 = ax1.imshow(concatenated_orig, aspect='auto',
                    cmap=seismic_cmap,
                    vmin=-vmax, vmax=vmax,
                    extent=[0, 360, concatenated_orig.shape[0], 0])
    ...
    ax1.set_title('Original Seismic - All Incidence Angles Concatenated')
    plt.colorbar(im1, ax=ax1, label='Amplitude')
    plt.tight_layout()
    st.pyplot(fig1)
    
    # Fluid-Substituted Seismic
    st.subheader("Fluid-Substituted Seismic - All Angles Concatenated")
    
    # Create concatenated array for fluid-substituted
    concatenated_sub = np.zeros((time_samples * n_angles, n_azimuths))
    
    for i in range(n_angles):
        start_row = i * time_samples
        end_row = (i + 1) * time_samples
        concatenated_sub[start_row:end_row, :] = results['seismic_sub'][i]
    
    # Plot concatenated fluid-substituted seismic
    fig2, ax2 = plt.subplots(figsize=(15, 8))
    vmax = np.abs(concatenated_sub).max()
    im2 = ax2.imshow(concatenated_sub, aspect='auto',
                    cmap=seismic_cmap,
                    vmin=-vmax, vmax=vmax,
                    extent=[0, 360, concatenated_sub.shape[0], 0])
    ...
    ax2.set_title('Fluid-Substituted Seismic - All Incidence Angles Concatenated')
    plt.colorbar(im2, ax=ax2, label='Amplitude')
    plt.tight_layout()
    st.pyplot(fig2)
    
    # Difference plot
    st.subheader("Seismic Difference - All Angles Concatenated")
    
    concatenated_diff = concatenated_sub - concatenated_orig
    vmax_diff = np.abs(concatenated_diff).max()
    
    fig3, ax3 = plt.subplots(figsize=(15, 8))
    im3 = ax3.imshow(concatenated_diff, aspect='auto',
                    cmap='RdBu',
                    vmin=-vmax_diff, vmax=vmax_diff,
                    extent=[0, 360, concatenated_diff.shape[0], 0])
    ...
    ax3.set_title('Seismic Difference (Fluid-Substituted - Original) - All Incidence Angles')
    plt.colorbar(im3, ax=ax3, label='Amplitude Difference')
    plt.tight_layout()
    st.pyplot(fig3)
...
def display_results(results, seismic_cmap, selected_angle):
    ...
    # 4. Concatenated Seismic Gathers
    plot_concatenated_seismic(results, seismic_cmap)
    
    # 5. Concatenated CWT Analysis
    plot_concatenated_cwt(results)
    
    # 6. Individual Seismic Gathers Comparison (Optional - can be removed if not needed)
    st.header("Individual Seismic Gathers Comparison")
