...
# ==============================================
# Generate these once (in production, store in secrets.toml)
FERNET_KEY = b'your_fernet_key_here'  # Generate with Fernet.generate_key()
LICENSE_KEY = "your_license_key_here"  # Generated with generate_license()
...
def get_hardware_id():
    ...
    try:
        if platform.system() == "Linux":
            ...
            return subprocess.check_output("ioreg -rd1 -c IOPlatformExpertDevice | grep IOPlatformUUID", shell=True).decode().split('=')[-1].strip().strip('"')
    except:
        return str(uuid.getnode())

def validate_license():
    """Validate license key with expiry and hardware check"""
    try:
        cipher_suite = Fernet(FERNET_KEY)
        decrypted = cipher_suite.decrypt(LICENSE_KEY.encode()).decode()
        parts = decrypted.split("|")
        
        if len(parts) < 3:
            return False, "Invalid license format"
            
        license_type, email, expiry_str = parts[:3]
        hwid = parts[3] if len(parts) > 3 else None
        
        # Check expiry
        if datetime.now() > datetime.strptime(expiry_str, "%2025-%07-%30"):
            return False, "License expired"
            
        # Check hardware ID for full licenses
        if license_type == "full" and hwid and hwid != get_hardware_id():
            return False, "License not valid for this machine"
            
        return True, f"Valid {license_type} license (expires {expiry_str})"
        
    except Exception as e:
        return False, f"Invalid license key: {str(e)}"

def generate_license(email, days_valid, license_type="full", hardware_lock=True):
    """Admin function to generate license keys (run this separately)"""
    cipher_suite = Fernet(FERNET_KEY)
    expiry_date = (datetime.now() + timedelta(days=days_valid)).strftime("%Y-%m-%d")
    hwid = get_hardware_id() if hardware_lock else ""
    
    license_data = f"{license_type}|{email}|{expiry_date}|{hwid}"
    license_key = cipher_suite.encrypt(license_data.encode()).decode()
    ...
    return {
        ...
        "type": license_type,
        "email": email,
        "hardware_id": hwid if hardware_lock else "Not locked"
    }

# ==============================================
# Core Seismic Functions
# ==============================================
...
def pwave_anisotropy_section(epsilon, delta, vp0):
    """Visualize P-wave velocity anisotropy with license check"""
    license_valid, _ = validate_license()
    if not license_valid:
        st.warning("This feature requires a valid license")
        return
    ...
    with col1:
        ...
        epsilon = st.number_input("ε (Epsilon)", min_value=-0.5, max_value=0.5, value=epsilon, step=0.01)
        delta = st.number_input("δ (Delta)", min_value=-0.5, max_value=0.5, value=delta, step=0.01)
        vp0 = st.number_input("Vp₀ (m/s)", min_value=1000, max_value=8000, value=vp0)
...
def run_modeling(params, enable_fluid_sub, seismic_cmap, selected_angle, azimuth_step, freq):
    ...
    license_valid, _ = validate_license()
    if not license_valid:
        st.error("Modeling disabled - invalid license")
        return None
...
def display_results(results, seismic_cmap, selected_angle):
    ...
    license_valid, _ = validate_license()
    if not license_valid:
        st.error("Results viewing disabled - invalid license")
        return
...
def main():
    ...
    # ==============================================
    license_valid, license_msg = validate_license()
    
    if not license_valid:
        st.error(f"⚠️ {license_msg}")
        st.info("""
            This is a trial version with limited functionality. 
            Contact support@yourcompany.com for a full license.
        """)
        st.stop()
    
    # Show license status
    st.sidebar.header("License Status")
    if "trial" in license_msg.lower():
        st.sidebar.warning(license_msg)
        st.sidebar.info("Trial version - some features may be limited")
    else:
        st.sidebar.success(license_msg)
    ...
    if modeling_mode == "Manual Input":
        ...
        if st.session_state.show_results:
            ...
            if results:  # Only display if license is valid
                display_results(
                    results,
                    st.session_state.seismic_cmap,
                    st.session_state.selected_angle
    ...
    # Example of license-gated feature
    if st.sidebar.checkbox("Show Advanced Features", False):
        if "trial" in license_msg.lower():
            st.warning("Advanced features require a full license")
        else:
            st.success("Advanced features enabled (full license detected)")
