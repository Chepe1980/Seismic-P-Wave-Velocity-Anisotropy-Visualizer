...
def run_fracture_modeling_app():
    ...
    with st.sidebar:
        ...
        # Add button to generate SWS data from fracture results
        if st.session_state.fracture_results is not None:
            st.markdown("### 🔀 Generate SWS Data")
            if st.button("Generate SWS Measurements for Tab 4"):
                sws_df = convert_fracture_results_to_sws(st.session_state.fracture_results)
                st.session_state.sws_from_fracture = sws_df
...
def convert_fracture_results_to_sws(fracture_results_df, epsilon_col='HUDSON_EPS', fracture_strike=90, q_scale=1.0, n_receivers=20):
    ...
    # Borehole receivers (4 boreholes × 5 receivers)
    ...
    for event_idx in range(n_events):
        ...
        for rec_idx, rec_loc in enumerate(receiver_locs):
            ...
            # Add realistic noise
            np.random.seed(event_idx * 100 + rec_idx)
            dt_noise = np.random.randn() * 0.0001
            dt = max(dt + dt_noise, 0)
            
            # Fast polarization aligned with fracture strike (with noise)
...
def generate_synthetic_sws_data(n_events=20, n_receivers=20):
    ...
    # Receiver locations (from thesis)
    receiver_locs = []
...
def plot_sws_results_plotly(df, analyzer, show_all_in_polar=True, q_threshold=0.5):
    ...
    if len(df_good) > 0:
        ...
        fig.add_trace(
            go.Scatter(
                x=df_good['phi'],
                y=[0.08] * len(df_good),  # placeholder density
                mode='markers',
                marker=dict(
                    size=10,
                    color=df_good['Q'],
            ...
            if len(df_null) > 0:
                ...
                fig_null.add_trace(go.Scatter(
                    ...
                    text=[f"Q: {q:.2f}<br>ϕ: {p:.1f}°" 
                          for q, p in zip(df_null['Q'], 
                                         df_null['phi'] if 'phi' in df_null.columns else [0]*len(df_null))],
                    hoverinfo='text'
                ))
                
                fig_null.update_layout(
                    title="Null Measurements Distribution",
                    xaxis_title="Azimuth (deg)" if 'azimuth' in df_null.columns else "Index",
                    yaxis_title="Inclination (deg)" if 'inclination' in df_null.columns else "Value",
                    height=500
                )
