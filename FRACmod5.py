...
def process_csv_data(uploaded_file, depth_ranges):
    """Process uploaded CSV file with individual layer depth ranges"""
    try:
...
def run_avaz_modeling_app():
    ...
    if 'csv_data_processed' not in st.session_state:
        st.session_state.csv_data_processed = False
    ...
    if modeling_mode == "Manual Input":
        ...
        st.session_state.modeling_mode = "csv"
        st.session_state.show_results = False
        
        with st.sidebar:
            st.header("CSV Import Settings")
            
            uploaded_file = st.file_uploader("Upload CSV file", type=["csv"])
            
            if uploaded_file is not None:
                try:
                    # Read CSV to get full depth range
                    df = pd.read_csv(uploaded_file)
                    ...
                    if 'DEPTH' not in df.columns:
                        st.error("CSV must contain 'DEPTH' column")
                        st.stop()
                    ...
                    freq = st.slider("Wavelet Frequency (Hz)", 10, 100, 45, key="csv_freq")
                    azimuth_step = st.slider("Azimuth Step (deg)", 1, 30, 10, key="csv_azimuth_step")
                    
                    st.subheader("Fluid Substitution Parameters")
                    enable_fluid_sub = st.checkbox("Enable Fluid Substitution", True, key="csv_fluid_sub")
                    if enable_fluid_sub:
                        phi = st.slider("Porosity (ϕ)", 0.01, 0.5, 0.2, 0.01, key="csv_phi")
                        Km = st.number_input("Mineral Bulk Modulus (GPa)", 10.0, 100.0, 37.0, 1.0, key="csv_Km")
                        Gm = st.number_input("Mineral Shear Modulus (GPa)", 10.0, 100.0, 44.0, 1.0, key="csv_Gm")
                        Kf = st.number_input("Fluid Bulk Modulus (GPa)", 0.1, 5.0, 2.2, 0.1, key="csv_Kf")
                        new_fluid_density = st.number_input("New Fluid Density (g/cc)", 0.1, 1.5, 1.0, 0.1, key="csv_fluid_density")
                    
                    show_anisotropy = st.checkbox("Show P-Wave Anisotropy Section", False, key="csv_show_anisotropy")
                    
                    if st.button("Run Modeling with CSV Data"):
                        st.session_state.csv_data_processed = True
                        st.session_state.show_anisotropy_section = show_anisotropy
        ...
        # Main workspace content for CSV mode
        if uploaded_file is not None and hasattr(st.session_state, 'depth_ranges'):
            ...
            if st.session_state.csv_data_processed:
                try:
                    # Process CSV data with individual layer ranges
                    params = process_csv_data(
...
class ShearWaveSplittingAnalyzer:
    """
    Implementation of automated shear-wave splitting analysis based on the thesis Chapter 4
    Uses cross-correlation and eigenvalue methods with cluster analysis (Teanby et al., 2004a)
    and quality factor Q (Wuestefeld et al., 2010)
    """
    
    def __init__(self, dt_sampling=0.001):
        """
        Initialize SWS analyzer
        
        Args:
            dt_sampling: Sampling interval in seconds
        """
        self.dt = dt_sampling
        self.null_directions = []
        
    def calculate_ray_coordinates(self, source_loc, receiver_loc):
        """
        Calculate ray geometry: azimuth and inclination from source to receiver
        
        Args:
            source_loc: (x, y, z) source location
            receiver_loc: (x, y, z) receiver location
            
        Returns:
            azimuth: azimuth in degrees from north
            inclination: inclination in degrees from vertical
            ray_length: ray path length
        """
        dx = receiver_loc[0] - source_loc[0]
        dy = receiver_loc[1] - source_loc[1]
        dz = receiver_loc[2] - source_loc[2]
        
        ray_length = np.sqrt(dx**2 + dy**2 + dz**2)
        
        # Azimuth from north (0° = North, clockwise)
        azimuth = np.degrees(np.arctan2(dx, dy)) % 360
        
        # Inclination from vertical (0° = vertical down)
        inclination = np.degrees(np.arccos(dz / ray_length)) if ray_length > 0 else 0
        
        return azimuth, inclination, ray_length
    
    def rotate_to_ray_frame(self, seismograms, source_loc, receiver_loc, p_pick_time):
        """
        Rotate seismograms from geographic (Z, N, E) to ray coordinates (L, SV, SH)
        Based on P-wave particle motion (Al-Anboori, 2005)
        
        Args:
            seismograms: 3-component seismograms [time, 3] (Z, N, E)
            source_loc: (x, y, z) source location
            receiver_loc: (x, y, z) receiver location
            p_pick_time: P-wave arrival time sample index
            
        Returns:
            seismograms_ray: rotated seismograms [time, 3] (L, SV, SH)
            rotation_params: rotation parameters used
        """
        # Calculate ray direction from source to receiver
        azimuth, inclination, _ = self.calculate_ray_coordinates(source_loc, receiver_loc)
        
        # Use a window around P-wave pick to determine polarization
        window_start = max(0, p_pick_time - 10)
        window_end = min(seismograms.shape[0], p_pick_time + 20)
        
        p_window = seismograms[window_start:window_end, :]
        
        # Compute polarization using eigenanalysis of covariance matrix
        cov_matrix = np.cov(p_window.T)
        eigenvalues, eigenvectors = np.linalg.eigh(cov_matrix)
        
        # Principal eigenvector (largest eigenvalue) gives P-wave polarization
        p_polarization = eigenvectors[:, np.argmax(eigenvalues)]
        
        # Rotation matrix from geographic to ray coordinates
        # L-axis: ray direction (from P-wave polarization)
        L = p_polarization / np.linalg.norm(p_polarization)
        
        # SH-axis: horizontal perpendicular to L
        # Cross product with vertical to get horizontal perpendicular
        vertical = np.array([1, 0, 0])  # Z-component is first in my data
        SH = np.cross(L, vertical)
        if np.linalg.norm(SH) < 1e-10:
            # If L is nearly vertical, use arbitrary horizontal
            SH = np.array([0, 1, 0])
        else:
            SH = SH / np.linalg.norm(SH)
        
        # SV-axis: completes right-handed system
        SV = np.cross(SH, L)
        SV = SV / np.linalg.norm(SV)
        
        # Rotation matrix
        rotation_matrix = np.vstack([L, SV, SH]).T
        
        # Apply rotation
        seismograms_ray = seismograms @ rotation_matrix
        ...
        return seismograms_ray, rotation_params
    
    def cluster_analysis(self, seismograms_ray, s_pick_time, window_params=None):
        """
        Perform cluster analysis for automated SWS measurement
        Based on Teanby et al. (2004a)
        
        Args:
            seismograms_ray: rotated seismograms [time, 3] (L, SV, SH)
            s_pick_time: S-wave arrival time sample index
            window_params: dictionary with window parameters:
                n_beg: number of start window variations
                n_end: number of end window variations
                t_beg_min: minimum start time before S-pick (samples)
                t_beg_max: maximum start time before S-pick (samples)
                t_end_min: minimum end time after S-pick (samples)
                t_end_max: maximum end time after S-pick (samples)
                
        Returns:
            results: dictionary with cluster analysis results
        """
        if window_params is None:
            window_params = {
                'n_beg': 5,
                'n_end': 6,
                't_beg_min': 5,   # samples before S-pick
                't_beg_max': 2,    # samples before S-pick
                't_end_min': 10,   # samples after S-pick
                't_end_max': 12    # samples after S-pick
            }
        
        # Generate windows
        t_beg_values = np.linspace(
            s_pick_time - window_params['t_beg_min'],
            s_pick_time - window_params['t_beg_max'],
            window_params['n_beg']
        ).astype(int)
        
        t_end_values = np.linspace(
            s_pick_time + window_params['t_end_min'],
            s_pick_time + window_params['t_end_max'],
            window_params['n_end']
        ).astype(int)
        
        # Ensure windows are within bounds
        t_beg_values = np.clip(t_beg_values, 0, seismograms_ray.shape[0]-10)
        t_end_values = np.clip(t_end_values, t_beg_values[0]+5, seismograms_ray.shape[0])
        
        results = {
            'windows': [],
            'phi_xc': [],  # fast polarization from cross-correlation
            'dt_xc': [],   # delay time from cross-correlation
            'phi_ev': [],  # fast polarization from eigenvalue method
            'dt_ev': [],   # delay time from eigenvalue method
            'quality': []  # quality for each window
        }
        
        for t_beg in t_beg_values:
            for t_end in t_end_values:
                if t_end <= t_beg:
                    continue
                    
                window_data = seismograms_ray[t_beg:t_end, 1:3]  # SV, SH components
                
                if len(window_data) < 5:
                    continue
                
                # Cross-correlation method
                phi_xc, dt_xc = self._cross_correlation_method(window_data)
                
                # Eigenvalue method
                phi_ev, dt_ev = self._eigenvalue_method(window_data)
                
                # Calculate quality for this window
                q_win = self._calculate_quality(phi_xc, dt_xc, phi_ev, dt_ev)
                
                results['windows'].append((t_beg, t_end))
                results['phi_xc'].append(phi_xc)
                results['dt_xc'].append(dt_xc)
                results['phi_ev'].append(phi_ev)
                results['dt_ev'].append(dt_ev)
                results['quality'].append(q_win)
        
        # Find stable region (plateau) in cluster analysis
        stable_results = self._find_stable_region(results)
        
        # Calculate overall quality
        q_best = np.max(results['quality'])
        q_avg = np.mean(results['quality'])
        q_event = (1/3) * q_avg + (2/3) * q_best
        
        return {
            'cluster_results': results,
            'stable': stable_results,
            'quality_event': q_event,
            'phi_best': stable_results['phi_best'],
            'dt_best': stable_results['dt_best']
        }
    
    def _cross_correlation_method(self, window_data):
        """
        Cross-correlation method for SWS measurement
        Based on Ando & Ishikawa (1980)
        
        Args:
            window_data: [n_samples, 2] array of SV, SH components
            
        Returns:
            phi: fast polarization direction (degrees)
            dt: delay time (seconds)
        """
        n_samples = len(window_data)
        max_dt_samples = min(30, n_samples // 4)
        
        best_corr = -1
        best_phi = 0
        best_dt = 0
        
        # Grid search over phi and dt
        for phi in np.arange(-90, 90, 5):
            phi_rad = np.radians(phi)
            
            # Rotate to candidate fast/slow directions
            rotated = np.zeros_like(window_data)
            rotated[:, 0] = window_data[:, 0] * np.cos(phi_rad) + window_data[:, 1] * np.sin(phi_rad)
            rotated[:, 1] = -window_data[:, 0] * np.sin(phi_rad) + window_data[:, 1] * np.cos(phi_rad)
            
            # Cross-correlate fast and slow
            for dt_samples in range(0, max_dt_samples):
                dt = dt_samples * self.dt
                
                if dt_samples == 0:
                    corr = np.corrcoef(rotated[:, 0], rotated[:, 1])[0, 1]
                else:
                    # Shift slow relative to fast
                    fast = rotated[dt_samples:, 0]
                    slow = rotated[:-dt_samples, 1]
                    if len(fast) > 5:
                        corr = np.corrcoef(fast, slow)[0, 1]
                    else:
                        corr = -1
        ...
        return best_phi, best_dt
    
    def _eigenvalue_method(self, window_data):
        """
        Eigenvalue method for SWS measurement
        Based on Silver & Chan (1991)
        ...
        for phi in np.arange(-90, 90, 5):
            ...
            for dt_samples in range(0, max_dt_samples):
                ...
                # Apply inverse splitting to linearize particle motion
                if dt_samples == 0:
                    corrected = window_data.copy()
                else:
                    # Apply time shift to slow component
                    corrected = np.zeros_like(window_data)
                    corrected[:n_samples-dt_samples, 0] = window_data[dt_samples:, 0]
                    corrected[dt_samples:, 1] = window_data[:-dt_samples, 1]
                
                # Rotate to candidate fast/slow
                rotated = np.zeros_like(corrected)
                rotated[:, 0] = corrected[:, 0] * np.cos(phi_rad) + corrected[:, 1] * np.sin(phi_rad)
                rotated[:, 1] = -corrected[:, 0] * np.sin(phi_rad) + corrected[:, 1] * np.cos(phi_rad)
                ...
                if ratio < min_eigenvalue_ratio:
                    min_eigenvalue_ratio = ratio
                    best_phi = phi
                    best_dt = dt
        
        return best_phi, best_dt
    
    def _calculate_quality(self, phi_xc, dt_xc, phi_ev, dt_ev):
        """
        Calculate quality factor Q for SWS measurement
        Based on Wuestefeld et al. (2010)
        
        Q ranges from +1 (good) to -1 (good null)
        """
        # Normalize parameters
        delta = dt_xc / dt_ev if dt_ev > 0 else 0
        omega = abs(phi_ev - phi_xc) / 45.0
        
        # Clamp values
        delta = min(max(delta, 0), 10)
        omega = min(omega, 1)
        
        # Calculate distances
        d_good = np.sqrt((delta - 1)**2 + omega**2)
        d_null = np.sqrt(delta**2 + (omega - 1)**2)
    ...
    def _find_stable_region(self, cluster_results):
        """
        Find stable region in cluster analysis results
        Based on Teanby et al. (2004a)
        """
        n_windows = len(cluster_results['windows'])
        if n_windows == 0:
            return {'phi_best': 0, 'dt_best': 0, 'stability': 0}
        
        # Convert to arrays
        phi_array = np.array(cluster_results['phi_xc'])
        dt_array = np.array(cluster_results['dt_xc'])
        quality_array = np.array(cluster_results['quality'])
        
        # Weight by quality
        weights = (quality_array + 1) / 2  # Scale from 0 to 1
        
        # Find stable region (where standard deviation is low)
        window_size = max(3, n_windows // 4)
        best_stability = np.inf
        best_start = 0
        
        for start in range(0, n_windows - window_size):
            phi_slice = phi_array[start:start+window_size]
            dt_slice = dt_array[start:start+window_size]
            
            phi_std = np.std(phi_slice)
            dt_std = np.std(dt_slice)
            
            stability = phi_std + dt_std * 100  # Scale dt std
            
            if stability < best_stability:
                best_stability = stability
                best_start = start
        
        # Use weighted median from stable region
        stable_end = best_start + window_size
        stable_phi = phi_array[best_start:stable_end]
        stable_dt = dt_array[best_start:stable_end]
        stable_weights = weights[best_start:stable_end]
        
        # Weighted median
        sorted_idx = np.argsort(stable_phi)
        phi_best = stable_phi[sorted_idx[len(sorted_idx)//2]]
        
        sorted_idx = np.argsort(stable_dt)
        dt_best = stable_dt[sorted_idx[len(sorted_idx)//2]]
    ...
    def classify_quality(self, Q):
        """
        Classify Q value into quality categories
        Based on thesis Chapter 4.3.1
        """
        if Q >= 0.75:
            ...
            return "Fair Null", "quality-null"
        else:
            return "Good Null", "quality-null"
    
    def calculate_delta_vs(self, dt, ray_length, vs_background):
        """
        Calculate percentage velocity anisotropy δVs
        
        Args:
            dt: delay time (seconds)
            ray_length: ray path length (meters)
            vs_background: background S-wave velocity (m/s)
            
        Returns:
            delta_vs: percentage velocity difference
        """
        if ray_length == 0 or vs_background == 0:
            return 0
        
        vs_fast = vs_background
        vs_slow = ray_length / (ray_length/vs_fast + dt)
        
        delta_vs = 100 * (vs_fast - vs_slow) / ((vs_fast + vs_slow) / 2)
        return delta_vs


def generate_synthetic_sws_data(n_events=20, n_receivers=10):
    """
    Generate synthetic SWS data for demonstration
    Based on thesis Chapter 4 model parameters
    """
    np.random.seed(42)
    
    # Model geometry from thesis
    source_loc = np.array([100, 150, 140])
    fracture_strike = 90  # degrees from north
    
    # Receiver locations (near-surface and borehole)
    receiver_locs = []
    
    # Surface array (49 receivers)
    for i in range(7):
        for j in range(7):
            x = 50 + i * 40
            y = 50 + j * 40
            z = 10  # Shallow
            receiver_locs.append([x, y, z])
    
    # Borehole arrays (4 boreholes, 5 receivers each)
    borehole_x = [80, 140, 200, 260]
    for x in borehole_x:
        for i in range(5):
            z = 100 + i * 40
            receiver_locs.append([x, 150, z])
    
    receiver_locs = np.array(receiver_locs[:n_receivers])
    
    data = []
    for event_idx in range(n_events):
        # Slight variation in fracture parameters
        event_strike = fracture_strike + np.random.randn() * 5
        event_density = 0.08 + np.random.randn() * 0.02
        
        for rec_idx, rec_loc in enumerate(receiver_locs):
            # Calculate ray geometry
            dx = rec_loc[0] - source_loc[0]
            dy = rec_loc[1] - source_loc[1]
            dz = rec_loc[2] - source_loc[2]
            
            ray_length = np.sqrt(dx**2 + dy**2 + dz**2)
            azimuth = np.degrees(np.arctan2(dx, dy)) % 360
            inclination = np.degrees(np.arccos(dz / ray_length))
            
            # Calculate expected SWS based on fracture model
            delta_phi = abs(azimuth - event_strike) % 180
            if delta_phi > 90:
                delta_phi = 180 - delta_phi
            
            # Stronger splitting when propagation is perpendicular to fractures
            splitting_factor = np.sin(np.radians(delta_phi))**2
            
            # Delay time proportional to ray length and fracture density
            dt_base = 0.001 * (ray_length / 100) * event_density / 0.08
            dt = dt_base * splitting_factor
            
            # Add noise
            dt += np.random.randn() * 0.0002
            
            # Fast polarization aligned with fracture strike
            phi = event_strike
            if delta_phi > 45:
                # Swap fast/slow for some azimuths
                phi = (phi + 90) % 180
            
            phi += np.random.randn() * 3
            
            # Calculate quality based on geometry
            Q = 0.9 * splitting_factor + 0.1 * np.random.randn()
            
            # Null when propagation is along fractures or polarizations aligned
            if splitting_factor < 0.1:
                Q = -0.9 + 0.1 * np.random.randn()
            ...
            data.append({
                ...
                'dt': dt,
                'Q': Q,
                'fracture_strike_true': event_strike,
                'fracture_density_true': event_density
            })
...
def plot_sws_results_plotly(df, analyzer):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                ...
                cmax=1,
                showscale=True,
                colorbar=dict(title="Q", x=0.28, y=0.78, len=0.3)
            ),
            text=[f"Q: {q:.2f}<br>ϕ: {p:.1f}°<br>δt: {dt*1000:.2f}ms" 
                  for q, p, dt in zip(df['Q'], df['phi'], df['dt'])],
            hoverinfo='text'
        ),
        row=1, col=1
    )
    ...
    fig.update_yaxes(title_text="Inclination (deg)", row=1, col=1)
    
    # Plot 2: Quality histogram
    fig.add_trace(
        go.Histogram(
            x=df['Q'],
            nbinsx=30,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=[f"ϕ: {p:.1f}°<br>Q: {q:.2f}" 
                  for p, q in zip(df['phi'], df['Q'])]
        ),
        row=2, col=2
    )
    
    fig.update_xaxes(title_text="Azimuth (deg)", row=2, col=2)
    fig.update_yaxes(title_text="Delay Time (ms)", row=2, col=2)
    
    # Plot 6: Fracture strike inversion (simplified)
    if len(df_good) > 0:
        # Compute average strike from good measurements
        avg_strike = np.mean(df_good['phi'])
        
        fig.add_trace(
            go.Scatter(
                x=df_good['phi'],
                y=df_good['fracture_density_true'],
                mode='markers',
                marker=dict(
                    size=10,
                    color=df_good['Q'],
...
def run_sws_analysis_app():
    ...
    st.markdown('<h2 class="sub-header">Automated SWS with Cluster Analysis & Quality Control</h2>', unsafe_allow_html=True)
    
    st.markdown("""
    <div class="info-box">
    This app implements automated shear-wave splitting analysis based on the thesis Chapter 4:
    - **Cluster Analysis**: Multiple windows to find stable SWS measurements (Teanby et al., 2004a)
    ...
    # Initialize session state
    if 'sws_analyzer' not in st.session_state:
    ...
    # Sidebar
    with st.sidebar:
        st.markdown('<h3 style="text-align: center;">⚙️ SWS Parameters</h3>', unsafe_allow_html=True)
        
        # Data source
        data_source = st.radio(
            "Data Source",
            ["Generate Synthetic Data", "Upload SWS Data"]
        )
        ...
        if data_source == "Upload SWS Data":
            uploaded_file = st.file_uploader(
                "Upload CSV file", 
                type="csv",
                help="CSV with columns: event_id, receiver_id, source_x/y/z, receiver_x/y/z, phi, dt"
            )
        ...
        n_beg = st.slider("Number of start windows", 3, 10, 5)
        n_end = st.slider("Number of end windows", 3, 10, 6)
    ...
    if st.session_state.sws_data is not None:
        ...
        fig_quality = go.Figure(data=[
            go.Bar(
                ...
                marker_color=['green' if 'Good' in q and 'Null' not in q 
                              else 'orange' if 'Fair' in q
                              else 'red' if 'Poor' in q
                              else 'blue' for q in quality_counts.index]
            )
        ])
        
        fig_quality.update_layout(
            title="SWS Quality Classification",
        ...
        st.download_button(
            label="📥 Download Results as CSV",
            data=csv,
            file_name="sws_analysis_results.csv",
            mime="text/csv"
        )
        
        # Analysis of null measurements
        with st.expander("🔍 Null Measurement Analysis"):
            df_null = df[df['Q'] < -0.5]
            if len(df_null) > 0:
                st.write(f"Found {len(df_null)} null measurements (Q < -0.5)")
                
                fig_null = go.Figure()
                
                fig_null.add_trace(go.Scatter(
                    x=df_null['azimuth'],
                    y=df_null['inclination'],
                    mode='markers',
                    marker=dict(
                        size=8,
                        color=df_null['phi'],
                    ...
                    text=[f"Q: {q:.2f}<br>ϕ: {p:.1f}°" 
                          for q, p in zip(df_null['Q'], df_null['phi'])],
                    hoverinfo='text'
                ))
                
                fig_null.update_layout(
                    title="Null Measurements Distribution",
                    xaxis_title="Azimuth (deg)",
                ...
                st.markdown("""
                **Interpretation of Null Measurements:**
                - Propagation parallel or perpendicular to fracture strike
                - Initial S-wave polarization aligned with symmetry plane
                - Isotropic medium (no fractures)
        ...
        This application implements automated shear-wave splitting analysis as described in the thesis Chapter 4.
        
        **Features:**
        - 🔄 **Automated Cluster Analysis**: Multiple windows to find stable measurements
        - 📊 **Dual Methods**: Cross-correlation and eigenvalue techniques
        ...
        **Quality Categories:**
        ...
        - **Fair (0.25 ≤ Q < 0.75)**: Acceptable but with some uncertainty
        - **Poor (-0.25 ≤ Q < 0.25)**: Unreliable - discard
        - **Fair Null (-0.75 ≤ Q < -0.25)**: Possibly null
        - **Good Null (Q < -0.75)**: Confident null - indicates no splitting
        
        **Null measurements** occur when:
        - Propagation is parallel or perpendicular to fracture strike
        - Initial polarization aligns with symmetry axis
        - Medium is isotropic
        
        **To get started:**
        1. Generate synthetic data or upload your own SWS measurements
        ...
        with st.expander("📋 Example CSV Format"):
            example_df = generate_synthetic_sws_data(n_events=2, n_receivers=5)
            st.dataframe(example_df.head(10))
            
            st.markdown("""
            **Required columns:**
            - `event_id`: Event/source identifier
            - `receiver_id`: Receiver identifier  
            - `source_x`, `source_y`, `source_z`: Source coordinates
            - `receiver_x`, `receiver_y`, `receiver_z`: Receiver coordinates
            - `phi`: Fast polarization direction (degrees)
            - `dt`: Delay time (seconds)
            - `Q`: Quality factor (optional, will be calculated if not provided)
            """)
