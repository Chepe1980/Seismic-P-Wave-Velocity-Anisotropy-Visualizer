...
def brown_korringa_substitution(Km, Gm, Ks, Gs, Kf, phi, delta, gamma):
    ...
    Gs: Solid matrix shear modulus (frame)
    Kf: Fluid bulk modulus
    phi: Porosity
    delta, gamma: Thomsen anisotropy parameters
    Returns: New bulk (K_sat), shear (G_sat) moduli, and updated anisotropy parameters
    """
    # Calculate dry rock moduli from input parameters
    K_dry = Ks
    G_dry = Gs
    
    # Brown-Korringa equations
    beta = 1 - (K_dry/Km)
    K_sat = K_dry + (beta**2) / ((phi/Kf) + ((beta - phi)/Km) - (delta*K_dry)/(3*Km))
    
    # Shear modulus affected by anisotropy (gamma parameter)
    G_sat = G_dry * (1 - (gamma*K_dry)/(3*Km))
    
    # Update anisotropy parameters (simplified approximation)
    delta_sat = delta * (K_sat/K_dry)
    gamma_sat = gamma * (G_sat/G_dry)
        ...
        with st.spinner("Computing full angle-azimuth response..."):
            ...
            if enable_fluid_sub:
                ...
                K_sat, G_sat, delta_sat, gamma_sat = brown_korringa_substitution(
                    ...
                    params['Kf']*1e9, 
                    params['phi'], 
                    params['dlt2'], params['g2']  # Using the layer's delta and gamma
                )
                ...
                vs[1] = Vs_new
                d[1] = new_density
                dlt[1] = delta_sat  # Updated delta
                g[1] = gamma_sat    # Updated gamma
                
                st.success(
                    f"Fluid substitution complete:\n"
                    f"- New Vp: {Vp_new:.1f} m/s (original: {params['vp2']:.1f})\n"
                    f"- New Vs: {Vs_new:.1f} m/s (original: {params['vs2']:.1f})\n"
                    f"- New Density: {new_density:.2f} g/cc (original: {params['d2']:.2f})\n"
                    f"- Updated δ: {delta_sat:.3f} (original: {params['dlt2']:.3f})\n"
                    f"- Updated γ: {gamma_sat:.3f} (original: {params['g2']:.3f})"
                )
