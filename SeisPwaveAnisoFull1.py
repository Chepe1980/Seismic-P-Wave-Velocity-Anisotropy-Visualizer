...
def process_excel_data(uploaded_file, depth_range):
    """Process uploaded Excel file and extract parameters for selected depth range"""
    try:
        # Read Excel file
    ...
    if modeling_mode == "Manual Input":
        ...
        with st.sidebar:
            ...
            if uploaded_file is not None:
                ...
                selected_angle = st.selectbox("Select angle for 2D comparison", 
                                            options=np.arange(0, max_angle+1, angle_step),
                                            index=3, key="excel_selected_angle")
                ...
                seismic_cmap = st.selectbox(
                    "Seismic Colormap",
                    options=['seismic', 'RdBu', 'bwr', 'coolwarm', 'viridis', 'plasma'],
                    index=0, key="excel_seismic_cmap"
                )
            ...
            if uploaded_file is not None and st.sidebar.button("Run Modeling with Excel Data"):
                # Process Excel data
                params = process_excel_data(uploaded_file, depth_range)
